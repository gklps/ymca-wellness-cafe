...
pub fn call_write_to_json_file(input_data: &AddActivityReq) -> Result<String, WasmError> {
    unsafe {
...
#[contract_fn]
pub fn add_activity(inp: AddActivityReq) -> Result<String, WasmError> {
    
    match call_write_to_json_file(&inp) {
        Ok(resp) => {
            Ok(resp)
        },
        Err(e) => {
